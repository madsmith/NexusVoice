...
class DummyContext:
    ...
    def __init__(self):
        ...
        logfire.info(f"Context created: {self.id}")
        self.entered = False
        self.exited = False
...
async def is_not_open(manager: RuntimeContextManager):
    ...
    assert manager._context_opened_at == -1, "Time should be unset when context is not open"
...
async def is_open(manager: RuntimeContextManager):
    ...
    assert manager._context_opened_at != -1, "Time should be set when context is opened"
...
async def test_runtime_context_manager_open_close(dummy_api):
    api = dummy_api
    manager = RuntimeContextManager(api, context_timeout=CONTEXT_TIMEOUT)
    manager.start()
...
async def test_context_manager_reopen_context_refreshes_timeout(dummy_api):
    ...
    assert ctx is not None
    ctx_id = ctx.id
    ctx_opened_at = manager._context_opened_at
    ...
    ctx_still_open = manager.get_context()
    assert ctx_still_open is not None, "Context should still exist before timeout"
    assert manager._context_opened_at is not None, "Context should have opened at"
    assert ctx_opened_at is not None, "Context should have opened at"
    
    assert ctx_opened_at < manager._context_opened_at, "Context should have been reacquired"
    ...
    ctx_expired = manager.get_context()
