...
def main():
    ...
    parser.add_argument("--chunk", type=int, default=1200, help="Initial chunk size")
    parser.add_argument("--measurements", type=int, default=3, help="Number of measurements to take")
    parser.add_argument("--step", type=int, default=200, help="Step size for chunk size")
    args = parser.parse_args()
    enable_graph = args.graph
    
    current_chunk_size = args.chunk
    current_volume = None

    search_step_size = args.step
    min_step_size = 1
    ...
    while search_step_size > min_step_size:
        if search_step_size < 100:
            measurements = 2
        if search_step_size < 10:
            measurements = 3
        if search_step_size < 3:
            measurements = 4

        left_chunk_size = max(current_chunk_size - search_step_size, 256)
        right_chunk_size = min(current_chunk_size + search_step_size, 8196)

        current_volume = test_chunk_size(current_chunk_size, measurements=measurements)
        left_volume = test_chunk_size(left_chunk_size, measurements=measurements)
        right_volume = test_chunk_size(right_chunk_size, measurements=measurements)
        
        print(f"Step: {search_step_size:2} | Left {left_chunk_size}: {left_volume:.2f} | Center {current_chunk_size}: {current_volume:.2f} | Right {right_chunk_size}: {right_volume:.2f}")

        if left_volume < current_volume and left_volume < right_volume:
            current_chunk_size, current_volume = left_chunk_size, left_volume
            if direction > 0:
                search_step_size = floor(search_step_size * 1.25)
            direction = -1
        elif right_volume < current_volume:
            current_chunk_size, current_volume = right_chunk_size, right_volume
            if direction < 0:
                search_step_size = floor(search_step_size * 1.25)
            direction = 1
        else:
            search_step_size = floor(search_step_size * .75)


    print(f"\nOptimized chunk_size: {current_chunk_size} → Avg Volume: {current_volume:.2f}")
